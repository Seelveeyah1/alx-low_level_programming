...
int main(int argc, char *argv[])
{
	...
	if (argc != 3)
		exit_with_error("Usage: cp source_file destination_file", 97);

	file_from = open(argv[1], O_RDONLY);

	if (file_from == -1)
		exit_with_error("Can't read from source file", 98);
	...
	{
		close(file_from);
		exit_with_error("Can't write to destination file", 99);
	}
